...
function Get-SystemInformation {
    ...
    try {
        ...
        $result += "Computer Name: $($info.CsComputerName)`n"
        $result += "Domain: $($info.CsDomain)`n"
        $result += "OS: $($info.OsName)`n"
        $result += "Install Date: $($info.OsInstallDate)`n"
        $result += "System Boot Time: $($info.OsLastBootUpTime)`n"
    } catch {
        $result += "Error retrieving system information: $($_.Exception.Message)`n"
    }
    return $result
}

function Get-OSDetails {
    $result = "=== OPERATING SYSTEM DETAILS ===`n"
    try {
        $os = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
        $result += "Caption: $($os.Caption)`n"
        $result += "Version: $($os.Version)`n"
        $result += "Build: $($os.BuildNumber)`n"
        $result += "Total Memory: $('{0:N0}' -f ($os.TotalVisibleMemorySize / 1MB)) MB`n"
        $result += "Free Memory: $('{0:N0}' -f ($os.FreePhysicalMemory / 1MB)) MB`n"
        $result += "Last Boot: $($os.LastBootUpTime)`n"
    } catch {
...
function Get-HardwareSummary {
    $result = "=== HARDWARE SUMMARY ===`n"
    try {
        $hw = Get-CimInstance Win32_ComputerSystem -ErrorAction Stop
        $result += "Manufacturer: $($hw.Manufacturer)`n"
        $result += "Model: $($hw.Model)`n"
        $result += "Processors: $($hw.NumberOfProcessors)`n"
        $result += "Logical Cores: $($hw.NumberOfLogicalProcessors)`n"
        $result += "RAM (GB): $('{0:N2}' -f ($hw.TotalPhysicalMemory / 1GB))`n"
        $result += "System Type: $($hw.SystemType)`n"
        $result += "Bootable: $($hw.Bootable)`n"
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
    return $result
...
function Get-CPUDetails {
    $result = "=== CPU INFORMATION ===`n"
    try {
        $cpu = Get-CimInstance Win32_Processor -ErrorAction Stop | Select-Object -First 1
        $result += "Name: $($cpu.Name)`n"
        $result += "Cores: $($cpu.NumberOfCores)`n"
        $result += "Logical Processors: $($cpu.NumberOfLogicalProcessors)`n"
        $result += "Speed (GHz): $('{0:N2}' -f ($cpu.MaxClockSpeed / 1000))`n"
        $result += "Architecture: $($cpu.Architecture)`n"
        $result += "Cache: $($cpu.L3CacheSize) KB`n"
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
    return $result
...
function Get-MemoryDetails {
    $result = "=== MEMORY DETAILS ===`n"
    try {
        $mem = Get-CimInstance Win32_PhysicalMemory -ErrorAction Stop
        $result += "Total Memory Modules: $($mem.Count)`n"
        foreach ($m in $mem) {
            $result += "Module: $($m.PartNumber) - $('{0:N2}' -f ($m.Capacity / 1GB)) GB - Speed: $($m.Speed) MHz`n"
        }
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
...
function Get-StorageSummary {
    ...
    try {
        ...
        foreach ($disk in $disks) {
            ...
            $result += "$($disk.Name) - $used/$total GB ($percent%)`n"
        }
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
...
function Get-NetworkConfiguration {
    $result = "=== NETWORK IP CONFIGURATION ===`n"
    try {
        $config = Get-NetIPConfiguration -ErrorAction Stop
        foreach ($cfg in $config) {
            $result += "Interface: $($cfg.InterfaceAlias)`n"
            $result += "  IPv4: $($cfg.IPv4Address.IPAddress -join ', ')`n"
            $result += "  Gateway: $($cfg.IPv4DefaultGateway.NextHopAddress -join ', ')`n"
            $result += "  DNS: $($cfg.DnsServer.Server -join ', ')`n"
            $result += "`n"
        }
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
...
function Get-NetworkAdapters {
    $result = "=== NETWORK ADAPTERS ===`n"
    try {
        $adapters = Get-NetAdapter -ErrorAction Stop
        foreach ($adapter in $adapters) {
            $speedGbps = '{0:N2}' -f ($adapter.Speed / 1000000000)
            $result += "Name: $($adapter.Name) | Status: $($adapter.Status) | Speed: $speedGbps Gbps`n"
        }
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
...
function Get-ActiveConnections {
    $result = "=== ACTIVE NETWORK CONNECTIONS (LISTEN) ===`n"
    try {
        ...
        foreach ($conn in $connections) {
            ...
            $result += "$($conn.LocalAddress):$($conn.LocalPort) - PID: $($conn.OwningProcess) ($($process.ProcessName))`n"
        }
    } catch {
        $result += "Error: $($_.Exception.Message)`n"
    }
